...
export const searchGif = async (context: any) => {
  ...
  if (keyword) {
    const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_API_KEY}&q=${keyword}`);
...
export const getTrendingGif = async (context: any) => {
  ...
  const response = await fetch(`https://api.giphy.com/v1/gifs/trending?api_key=${GIPHY_API_KEY}`);
  if (response) {
    result = await response.json();
  }
...
export const getGifFromId = async (context: any) => {
  const id = context.params.id;

  let result = null;

  if (id) {
    const response = await fetch(`https://api.giphy.com/v1/gifs/${id}?api_key=${GIPHY_API_KEY}`);
    if (response) {
      result = await response.json();
    }
  }
